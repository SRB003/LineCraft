$(function () {
  ...
  $(document).ready(function () {
    $("#assets").on("input", function () {
      ...
      if (value < 1 || value > 1000) {
        ...
        alert(
          "Please enter a value between 1 - 1000 for the number of machines."
        );
  ...
  $(document).ready(function () {
    $("#turnover").on("input", function () {
      ...
      if (value < 0 || value > 500000000) {
        // If the entered value is outside the range, show a message and set it to the nearest limit
        alert(
          "Please enter a value between $1000000 - $500000000 for the number of machines."
        );
      ...
      return Math.ceil(
        25 *
          (month -
            1 -
            previousCumulativeCashFlow /
              (cumulativeCashFlow - previousCumulativeCashFlow))
      );
    } else {
      return 0;
    }
  ...
  function calculateCostAndSubscription() {
    ...
    for (var month = 1; month <= 50; month++) {
      ...
      var breakevenDays = calculateBreakevenDays(
        month,
        cumulativeCashFlow,
        previousCumulativeCashFlow
      );
      ...
      if (cumulativeCashFlow > 0 && previousCumulativeCashFlow < 0) {
        // Corrected breakeven day calculation
        console.log(
          "Breakeven Days (Month " + month + "): " + breakevenDays.toFixed(2)
        );
        ...
        if (month === 12) {
          ...
          var ROI =
            (cumulativeCashFlow / (oneTimeCost + yearlySubscription)) * 100;
